...
function validate (context, request) {
  ...
  var wrapError = context.wrapValidationError || wrapValidationError

  if (params) {
    return wrapError(params, 'params')
  }
  var body = validateParam(context[bodySchema], request, 'body')
  if (body) {
    return wrapError(body, 'body')
  }
  var query = validateParam(context[querystringSchema], request, 'query')
  if (query) {
    return wrapError(query, 'querystring')
  }
  var headers = validateParam(context[headersSchema], request, 'headers')
  if (headers) {
    return wrapError(headers, 'headers')
  }
  return null
}
